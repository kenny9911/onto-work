...
export const PromptInput = ({
  ...
  const rejectAttachmentAttempt = useCallback(
    () => onAttachmentAttempt?.(),
    [onAttachmentAttempt]
  );
  ...
  const rejectAttachments = useCallback(
    (fileList: File[] | FileList) => {
      if (fileList.length > 0) rejectAttachmentAttempt();
    },
    [rejectAttachmentAttempt]
  );
  ...
  const add = attachmentsDisabled
    ? rejectAttachments
    : usingProvider
      ? addWithProviderValidation
      : addLocal;
  const remove = usingProvider ? controller.attachments.remove : removeLocal;
  const openFileDialog = attachmentsDisabled
    ? rejectAttachmentAttempt
    : usingProvider
...
export const PromptInputSubmit = ({
  ...
  if (status === "submitted") {
    ...
    Icon = canStop ? <SquareIcon className="size-4" /> : <Spinner />;
  ...
  return (
    <InputGroupButton
      aria-label={canStop ? "Stop" : isGenerating ? "Sending" : "Submit"}
      className={cn(className)}
      onClick={handleClick}
      size={size}
...
export const PromptInputTabLabel = ({
  ...
  <h3
    className={cn(
      "mb-2 px-3 font-medium text-ui-control text-muted-foreground",
      className
    )}
    {...props}
  />
...
export const PromptInputTabItem = ({
  ...
  <div
    className={cn(
      "flex items-center gap-2 px-3 py-2 text-ui-control hover:bg-accent",
      className
    )}
    {...props}
  />
